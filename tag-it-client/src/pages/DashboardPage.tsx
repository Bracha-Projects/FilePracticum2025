        ...
        {statsData.map((stat, index) => (
          <Card key={index} className="glass animate-fade-in" style={{ animationDelay: `${index * 100}ms` }}>
            <CardHeader className="pb-2 flex flex-row items-center justify-between space-y-0">
              <CardTitle className="text-sm font-medium text-tagit-darkblue">
                {stat.title}
              </CardTitle>
              <div className="p-2 bg-tagit-mint/10 rounded-full">
                {stat.icon}
              </div>
            </CardHeader>
            <CardContent>
              ...
              {stat.percentage !== undefined && (
                <div className="mt-2">
                  ...
                  <p className="text-xs text-tagit-blue mt-1">
                    {stat.percentage}% of 10 GB
                  </p>
                </div>
              )}
            </CardContent>
                  ...
                  <div key={index} className="flex items-start space-x-3 animate-fade-in" style={{ animationDelay: `${index * 100}ms` }}>
                    ...
                    <div>
                      <p className="text-sm text-tagit-darkblue">
                        <span className="font-medium">{activity.user}</span> {activity.action.toLowerCase()} <span className="font-medium">{activity.file}</span>
                      </p>
            ...
            <CardContent>
              <div className="flex flex-wrap gap-2">
                {popularTags.map((tag, index) => (
                  ...
                  >
                    <span>{tag.name}</span>
                    <span className="ml-2 text-xs bg-tagit-mint/40 px-1.5 py-0.5 rounded-full">
                      {tag.count}
                    </span>
                  </div>
                ))}
